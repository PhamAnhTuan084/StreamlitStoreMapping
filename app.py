...
def tao_danh_sach_thoa_khongthoa_2(teleco1, teleco2, HVN_phone):
    teleco1['Đầu Số Cũ'] = '0' + teleco1['Đầu Số Cũ'].astype(str)
    teleco1['Đầu Số Mới'] = '0' + teleco1['Đầu Số Mới'].astype(str)
    ...
    HVN_dausomoi, HVN_nodausomoi= check_dausomoi(HVN_phone, teleco1)
    HVN_mavungmoi, HVN_nomavungmoi= check_mavungmoi(HVN_nodausomoi, teleco2)

    HVN_khongthoa = HVN_nomavungmoi.copy()
    HVN_thoa = pd.concat([HVN_dausomoi, HVN_mavungmoi])
...
def round2_motfile(data1):
    ...
    result_df = result[result['OutletID_1'] != result['OutletID_2']]
    result_df['fuzzy_similarity'] = result_df.apply(fuzzy_similarity_2, axis=1)
...
def round3_motfile(HVN_r3):
    ...
    result_df['Score_Distance'] = result_df.apply(calc_score_dist_2, axis=1)
    result_df['Score_Name'] = result_df.apply(calc_score_name_3, axis=1)
...
def round4_motfile(HVN_r4):
    ...
    if merged_df.empty:
        ...
        filtered_result = merged_df[merged_df.apply(apply_filter_2, axis=1)]
        filtered_result['Score_Name_2'] = filtered_result.apply(calc_score_name_4, axis=1)
        storename80 = filtered_result.loc[filtered_result['Score_Name_2'] >= 80]
    return storename80
...
def Loc_File_2(df):
    ...
    # Lưu lại thứ tự của OutletID_file1 sau khi sắp xếp
    outlet1_order = df['OutletID_1'].unique()

    # Groupby theo 'OutletID_file1' với tham số sort=True để giữ nguyên thứ tự
    grouped_outlet1 = df.groupby('OutletID_1', sort=True)
...
def process_uploaded_motfile(uploaded_files):
    dataframes = {}
    HVN = None
...
def main():
    ...
    # Display the option_menu in the second column
    with col2:
        selected = option_menu(
            menu_title=None,
            options=["One File", "Two Files"],
            icons=["file", "files"],
            menu_icon="cast",
            default_index=0,
            orientation="horizontal",
            styles={
                "container": {"padding": "0!important", "background-color": "#fafafa", "max-width": "450px", "height": "45px"},
                "icon": {"color": "#b9bab8", "font-size": "25px"},
                "nav-link": {
                    "font-size": "25px",
                    "text-align": "left",
                    "margin": "0px",
    ...
    if selected == "One File":
        ...
        # Display files
        st.header("2. Display Uploaded File")

        dataframes = {}
        HVN = None

        if uploaded_files:
            dataframes, HVN = process_uploaded_motfile(uploaded_files)

            # Display information for HVN
            if HVN is not None:
                st.subheader("Displaying file:")
                st.dataframe(HVN)
        ...
        if st.button("Apply"):
            ...
            if HVN is not None:
                ...
                # Danh sách số điện thoại thỏa và không thỏa
                HVN_phone = xuly_phone_2(HVN)
                HVN_thoa, HVN_khongthoa = tao_danh_sach_thoa_khongthoa_2(teleco1, teleco2, HVN_phone)

                st.subheader("Displaying file after checking phone:")
                st.dataframe(HVN_thoa)
                ...
                if phonenum_map.empty:
                    ...
                    # Lọc data cho round2_motfile 
                    HVN_r2 = HVN_thoa.loc[lambda df: ~df.OutletID.isin(phonenum_map.OutletID_1)]
                    HVN_r2 = pd.concat([HVN_r2, HVN_khongthoa])

                # Lấy danh sách round2_motfile, danh sách thỏa gồm đầy đủ số nhà và tên đường và danh sách không thỏa do thiếu thông tin
                HVN_r2_thoa, HVN_r2_khonghtoa = loc_hvn_r2(HVN_r2)
                df1 = xuly_address_hvn(OptionalText, HVN_r2_thoa, text_remove)
                df1 = tao_address(df1)
                ...
                # Round 2: 100% address
                matching_addess = round2_motfile(df1)
                st.markdown('<h3 style="display:flex; align-items:center;">&hybull; Displaying round 2:</h3>', unsafe_allow_html=True)
                st.dataframe(matching_addess)
                ...
                if matching_addess.empty:
                    ...
                    # Lọc data cho round3_motfile
                    HVN_r3 = df1.loc[lambda df: ~df.OutletID.isin(matching_addess.OutletID_1)]
                ...
                location90storename100 = round3_motfile(HVN_r3_without_NoName)
                st.markdown('<h3 style="display:flex; align-items:center;">&hybull; Displaying round 3:</h3>', unsafe_allow_html=True)
                st.dataframe(location90storename100)       
                ...
                if location90storename100.empty:
                    ...
                    # Lọc data cho round 4
                    HVN_r4 = HVN_r3_without_NoName.loc[lambda df: ~df.OutletID.isin(location90storename100.OutletID_1)]
                # print(HVN_r4.info())s)
                ...
                if matching_addess.empty and phonenum_map.empty and location90storename100.empty and danh_sach_1.empty:
                    ...
                    df2 = pd.concat([HVN_r5, HVN_r3_with_NoName])
                    st.markdown('<h3 style="display:flex; align-items:center;">&cir; List of unsatisfactory Outlets for 4 rounds:</h3>', unsafe_allow_html=True)
                    st.dataframe(df2)
